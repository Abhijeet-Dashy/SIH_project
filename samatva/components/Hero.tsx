...
const Hero: React.FC = () => {
  ...
  return (
    <section className="relative h-[85vh] min-h-[650px] w-full flex items-center justify-center text-center text-white overflow-hidden">
      {/* Background */}
      <div 
      ...
      <div className="absolute bottom-0 left-0 right-0 h-48 bg-gradient-to-t from-brand-surface to-transparent z-5"></div>

      {/* Foreground content */}
      <motion.div 
        className="relative z-10 flex flex-col items-center px-4 -mt-24"
        initial={{ opacity: 0, y: 24 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.8 }}
      >
        <h1 className="font-serif text-4xl md:text-6xl font-bold text-brand-text">Your Sanctuary For Student Well-being</h1>
        <p className="mt-4 text-lg md:text-xl text-brand-text-light max-w-2xl">
          You don't have to hide your struggles behind a smile. This is your safe space to share, heal, and grow.
        </p>

        <div className="mt-12 w-full max-w-2xl">
          <motion.div className="relative bg-white/25 backdrop-blur-3xl border border-white/10 rounded-4xl p-3 shadow-xl shadow-black/10 overflow-hidden" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.6 }}>
            {/* Reflection */}
            <div className="absolute inset-0 bg-gradient-to-br from-white/20 via-white/10 to-transparent pointer-events-none"></div>
            
            <div className="relative">
              <span className={`absolute top-3 left-3 text-lg text-brand-text-light pointer-events-none ${input ? 'opacity-0' : 'opacity-100'}`}>
                {typedPlaceholder}
                <span className="ml-1 inline-block align-middle w-[2px] h-[1.2em] bg-brand-text-light opacity-70 animate-pulse"></span>
              </span>
              <textarea
                value={input}
                ...
                className="w-full h-28 bg-transparent text-brand-text placeholder-brand-text-light border-none focus:ring-0 resize-none p-3 text-lg"
              />
              <div className="absolute bottom-3 right-3 flex items-center space-x-2">
                <motion.button whileHover={{ scale: 1.1 }} className="p-2 text-brand-text-light hover:text-brand-text transition-colors"><PaperclipIcon /></motion.button>
                <motion.button whileHover={{ scale: 1.1 }} className="p-2 text-brand-text-light hover:text-brand-text transition-colors"><MicIcon /></motion.button>
                <motion.button
                  onClick={handleSubmit}
                  disabled={isLoading}
                  className={`bg-brand-text text-white rounded-full p-3 hover:bg-opacity-90 disabled:bg-gray-500 transform active:scale-95 transition-all duration-200 ${isFocused && input ? 'animate-subtle-pulse' : ''}`}
